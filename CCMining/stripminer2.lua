...
local function returnToInitialCoordinates(initialPos)
    if initialPos then
        turtle.moveTo(initialPos.x, initialPos.y, initialPos.z)
    end
end
    ...
    for i = -2, 2 do
        for j = -2, 2 do
            ...
            if block and block.name == "minecraft:chest" then
                local chestX, chestY, chestZ = turtle.getX(), turtle.getY(), turtle.getZ()
...
local function depositItems(chestPos)
    if chestPos then
        turtle.moveTo(chestPos.x, chestPos.y, chestPos.z)
        for i = 2, 16 do
            turtle.select(i)
            turtle.drop()
        end
